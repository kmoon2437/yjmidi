const MidiTrack = require('./MidiTrack.js');
...
module.exports = class MidiFile {
    ...
    constructor(data, tailMs = 500) {
        ...
        trackChunks.forEach(chunk => {
            ...
            while (dd.isDataAvailable) {
                ...
                if ((type & 0xf0) == 0xf0) {
                    ...
                    } else if (type == Consts.events.types.SYSEX || type == Consts.events.types.ESCAPE) {
                        length = dd.readVarUint();
                        data = dd.readBytes(length);
                    }
                } else {
        ...
        if (this.header.ticksPerBeat) {
            ...
            this.header.durationMs +=
                currentTempo * ((this.header.durationTick - currentTick) / this.header.ticksPerBeat);
